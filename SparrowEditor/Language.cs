	...
	public static class Language {
		...
		public static void Load() {
			...
			try {
				...
				langs = dict;
				Debug.WriteLine($"Loaded resources! [{CultureInfo.CurrentUICulture}]");
			}
			catch { // Fallback: Use one language selected file.
				var dict = new ResourceDictionary {
					Source = new Uri("Lang/pt-BR.xaml", UriKind.Relative) // pt-BR by now, cause its my main language lol
				};
				Application.Current.Resources.MergedDictionaries.Add(dict);
				langs = dict;
				Debug.WriteLine($"Failed to load resources in current UI Culture! [{CultureInfo.CurrentUICulture}]");
			}
		...
		public static string GetString(string key, params object[] value) {
			...
			if (langs == null || !langs.Contains(key)) {
				Debug.WriteLine($"ERROR: The current language doesn't have the expected key ({key}) or dictionary is null.");
				return $"[! {key} !]";
			}
